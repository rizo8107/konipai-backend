...
export const pb = new PocketBase(SERVER_ENV.POCKETBASE_URL);
...
export const initAdmin = async (retryCount = 0) => {
  try {
    ...
    const authPromise = pb.admins.authWithPassword(
      SERVER_ENV.POCKETBASE_ADMIN_EMAIL,
      SERVER_ENV.POCKETBASE_ADMIN_PASSWORD
    );
...
export const getImageUrl = (collectionId: string, recordId: string, fileName: string) => {
  ...
  return `${SERVER_ENV.POCKETBASE_URL}/api/files/${collectionId}/${recordId}/${fileName}`;
};
