...
import type { Order as OrderType } from '../types/schema';
...
function getWhatsAppApiUrl(): string {
  ...
  const apiUrl = SERVER_ENV.WHATSAPP_API_URL;
  ...
  if (!apiUrl) {
    console.warn('WHATSAPP_API_URL not found in environment. Using default API URL:', defaultApiUrl);
    return defaultApiUrl;
  }
  
  // Use direct URL approach
  console.log('Using WhatsApp API URL:', apiUrl);
  return apiUrl;
}
...
// Get the WhatsApp API URL from environment
const whatsappApiUrl = SERVER_ENV.WHATSAPP_API_URL;
